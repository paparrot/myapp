...
// Объявляем переменную $postId в запросе
    ...
    useEffect(() => {
        async function loadComments() {
            ...
            try {
                ...
                }>(GET_COMMENTS, { postId: String(postId) }); // Преобразуем number в string, так как в GraphQL ID - это строка
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {comments.map((comment) => (
                        <div
                            key={comment.id}
                            className="border rounded-xl px-4 py-3 bg-gray-50 dark:bg-gray-800 dark:border-slate-600 shadow-sm"
                        >
                            <p className="font-semibold py-0 my-0">{comment.author.node.name}</p>
                            <p
